...
def train():
    print("=" * 60)
    print("  LSTM Autoencoder — REFIT House 1")
    ...
    save_name = "autoencoder_lstm_refit_house1_best.pth"
...
def plot_training_curve(train_losses, val_losses, best_epoch):
    ...
    ax.set_xlabel('Epoch')
    ax.set_ylabel('MSE')
    ax.set_title('Training Curve — LSTM Autoencoder (House 1)', fontweight='bold')
    ax.legend()
    ax.grid(alpha=0.3)
    plt.tight_layout()
    save_fig('03_training_curve_house1.png')
...
def plot_error_distribution(train_errors, threshold):
    ...
    ax.grid(alpha=0.3)
    plt.tight_layout()
    save_fig('04_error_distribution_house1.png')
...
def plot_reconstruction(model, X_val, val_errors):
    ...
    plt.suptitle('LSTM Autoencoder Reconstruction — House 1',
                 fontsize=12, fontweight='bold')
    plt.tight_layout()
    save_fig('06_reconstruction_house1.png')
