...
def train():
    print("=" * 60)
    print("  LSTM Autoencoder — REFIT Dataset")
    print("=" * 60)
    ...
    val_df.to_csv(os.path.join(VAL_DIR, "lstm_refit_validation_dataset.csv"), index=False)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if avg_val < best_val_loss:
            ...
            torch.save(model.state_dict(),
                       os.path.join(SAVE_PATH, "autoencoder_lstm_refit_best.pth"))
            print("  ← best ")
        else:
            epochs_no_improve += 1
            print()
    ...
    print(f"  Model saved   : autoencoder_lstm_refit_best.pth")
    print(f"{'='*60}")
